...
function populatePayments(remittance) {
  ...
  // var strDate  = new Date('11/1/2021');
  var strDate = new Date();
  strDate.setDate(strDate.getDate() - 180);
  var endDate  = new Date();
  // pDate = today.toLocaleDateString("en-US", options);
 // console.log ('Start Date Range: ' + stDate.toString());


  db.collection("Payments").where('Remittance', '==', remittance)
  ...
  .onSnapshot(snapshot => {
    ...
    // console.log ("populatePayments: " + remittance);
    ...
    changes.forEach(change => {
      if (change.type == 'added') {
        ...
        var options = { year: 'numeric', month: 'short', day: 'numeric' };
        // console.log("Payment Date: " +  change.doc.data().PaymentDate);
        // console.log ('Payment Date: ' + new Date(change.doc.data().PaymentDate.seconds * 1000));
        var pDate = new Date(change.doc.data().PaymentDate.seconds * 1000);
        pDate = pDate.toLocaleDateString("en-US", options);
