...
export default function UserProfileRegisteredEvent(props) {
    ...
    const [filteredEvent, setFilteredEvent] = useState(props.eventList);
    ...
    function searchEvent() {
        ...
        var newArray = props.eventList.filter(function (el) {
            return el.EventName.toUpperCase().includes(searchKey.toUpperCase());
        }
        );
        // console.log(newArray);
        setFilteredEvent(newArray);
    }
            ...
            {props.selectedPlayer !== '' && <>
                ...
                <div style={{ display: 'block' }}>
                    ...
                    <div className='row no-gutters'>
                        ...
                        {props.participantList && props.participantList.map((participant) => {
                            obj = filteredEvent.find(e => e.EventID === participant.EventID);
                            if (obj && obj.EventStatus !== 'Active'
                                && participant.ParticipantID === props.selectedPlayer) {
                                ...
                                return <div className='col-lg-4 col-md-6 col-sm-12' key={participant.EventID + participant.PlayerID + participant.CategoryName}>
                                    <div className="event-list-card" style={{ height: '200px', borderColor: participant.PaymentStatus === 'Pending' ? 'orange' : 'green' }}>
                                        ...
                                        <div className="second-div">
                                            ...
                                            {participant.PaymentStatus === 'Completed' && <button
                                                className="mybutton button5"
                                                style={{ margin: '0', background: 'green', pointerEvents: 'none' }}>Paid</button>
                                            }
                                            {participant.PaymentStatus === 'Pending'
                                                &&
                                                <>
                                                    <button className="mybutton button5"
                                                        style={{ margin: '0', background: 'orange', pointerEvents: 'none' }}>
                                                        Pending
                                                    </button>
                                                    <br />
                                                </>
